      ...
      <div className="home-root">
        ...
        <section className="hero">
          <div className="canvas hero-grid">
            ...
            <div className="hero-side">
              <p>Here&apos;s the deal. I made a skill, one small file, that knows how to analyse a spreadsheet properly. I&apos;ll give you the skill and the prompt. The sample data is baked into the skill, so you don&apos;t even need a spreadsheet to try it.</p>
        ...
        <section className="steps">
          <div className="canvas">
            ...
            {/* Cowork note — this path is for people who already have Cowork */}
            <p className="cowork-note">
              You&apos;ve already got Cowork, Anthropic&apos;s desktop app for Claude. Don&apos;t have it yet? <a href="https://claude.ai/download" target="_blank" rel="noopener">Download Cowork ↗</a>
            </p>

            {/* STEP 01 — Download skill (entire row clickable) */}
            <a
              id="step-1"
            ...
            >
              ...
              <div className="step-body">
                ...
                <p>One small zip, with sample data built in. Drop it into Cowork and say &ldquo;install this skill&rdquo;. This file is where the magic lives.</p>
              </div>
              <div className="step-action">
                <span className={`row-badge ${is(1) ? 'done' : ''}`}>
                  {is(1) ? '✓ Downloaded' : '↓ csv-analyzer.zip'}
                </span>
              </div>
            </a>
            ...
            {/* STEP 03 — Copy prompt */}
            <div id="step-3" className={`step-row step-row-prompt ${is(3) ? 'done' : ''}`}>
              <div className="step-num">{is(3) ? '✓' : '03'}</div>
              <div className="step-body">
                <h3>Copy this prompt, paste it in</h3>
                <p>One line. Cowork picks up the skill and runs the whole analysis on its own. No file? It uses the built-in sample.</p>
                <div
                  className={`prompt ${is(3) ? 'done' : ''}`}
                  onClick={handleCopy}
                  role="button"
                  tabIndex={0}
                ...
                >
                  ...
                  <span className={`copy-pill ${copied ? 'copied' : ''} ${is(3) ? 'done' : ''}`}>
                    {copied ? '✓ Copied' : is(3) ? '✓ Done' : 'Click to copy'}
                  </span>
                </div>
              </div>
            </div>
        ...
        <section className="next">
          <div className="canvas">
            <div className="next-head">
              <h2>You just ran one skill. <span className="accent">Now see it for your own job.</span></h2>
...
const HOMEPAGE_STYLES = `
  ...
  .home-root .cowork-note {
    font-size: 14px; color: var(--ink-mid); margin: 0 0 4px; padding-bottom: 12px;
  }
  .home-root .cowork-note a { color: var(--orange); font-weight: 600; border-bottom: 1px solid transparent; }
  .home-root .cowork-note a:hover { border-bottom-color: var(--orange); }
  .home-root .step-row {
    display: grid;
  ...
  .home-root .next-head { margin-bottom: 24px; }
  .home-root .next-head h2 {
    font-family: var(--font-oxanium), sans-serif; font-weight: 700;
    font-size: clamp(28px, 3.4vw, 46px); line-height: 1.04;
    letter-spacing: -0.025em; color: var(--ink); margin: 0 0 8px;
  }
  .home-root .next-head h2 .accent { color: var(--orange); }
  .home-root .next-head p {
    font-size: 16px; line-height: 1.5; color: var(--ink-mid); margin: 0; max-width: 680px;
  }
  .home-root .next-grid {
