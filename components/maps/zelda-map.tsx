...
// Stats widget styled as a Zelda dungeon inventory panel
      ...
      {expanded && (
        <div
          ...
          style={{
            ...
            boxShadow: '0 2px 12px rgba(218, 165, 32, 0.15)',
          }}
        >
          <div
          ...
          <div className="space-y-2.5 text-xs" style={{ fontFamily: 'var(--font-oxanium), sans-serif' }}>
            <div className="flex justify-between gap-6">
              <span style={{ color: 'var(--zelda-parchment)', opacity: 0.7 }}>Treasures</span>
              <span className="font-bold" style={{ color: 'var(--zelda-gold)' }}>{skillCount}</span>
            </div>
            <div className="flex justify-between gap-6">
              <span style={{ color: 'var(--zelda-parchment)', opacity: 0.7 }}>Rooms cleared</span>
              <span className="font-bold" style={{ color: 'var(--zelda-gold)' }}>{completedCount}/{availableCount}</span>
            </div>
            <div className="flex justify-between gap-6">
              <span style={{ color: 'var(--zelda-parchment)', opacity: 0.7 }}>Time saved</span>
...
export default function ZeldaMap({
  ...
  return (
    <div className="page-enter skin-zelda" style={{ minHeight: 'calc(100vh - 3.5rem)' }}>
      ...
      >
        <img
          src={`/images/maps/zelda-level-${bgLevel}.png`}
          alt=""
          className="w-full h-full"
          style={{ objectFit: 'cover', objectPosition: 'center 40%' }}
        />
        {/* Dark forest vignette overlay for depth */}
        <div
          className="absolute inset-0"
          style={{
            background: 'radial-gradient(ellipse at center 30%, transparent 30%, rgba(16, 40, 16, 0.35) 100%)',
          }}
        />
      </div>
      ...
      <div className="relative z-10 min-h-[calc(100vh-3.5rem)] flex flex-col">
        ...
        <div className="flex-1 flex flex-col justify-center items-center gap-6">
          ...
          {/* Other dungeons (completed or locked) */}
          <div className="flex flex-col items-center gap-3">
            {[2, 1, 0].filter(idx => idx !== activeLevelIdx).map((idx, i) => {
              const done = isLevelComplete(idx + 1)
              return (
                <div key={idx} className="zelda-chapter-hint flex items-center gap-3" style={{ animationDelay: `${1 - i * 0.5}s` }}>
                  <div style={{ width: 40 + i * 10, height: 1, background: 'var(--zelda-gold)', opacity: done ? 0.5 : 0.3 + i * 0.1 }} />
                  <span
          ...
          {/* Active dungeon title */}
          <div className="flex flex-col items-center">
            <div className="flex items-center gap-4 mb-3">
              <div style={{ width: 80, height: 1, background: 'var(--zelda-gold)', opacity: 0.6 }} />
              {/* Triforce diamond ornament */}
              <svg width="20" height="18" viewBox="0 0 16 14" style={{ opacity: 0.6 }}>
                <polygon points="8,0 12,7 8,14 4,7" fill="var(--zelda-gold)" />
              </svg>
              <div style={{ width: 80, height: 1, background: 'var(--zelda-gold)', opacity: 0.6 }} />
            </div>
            <h2
              ...
              style={{
                ...
                fontFamily: 'var(--font-oxanium), sans-serif',
                fontWeight: 700,
                textShadow: '0 2px 12px rgba(16, 40, 16, 0.7)',
              }}
            >
              ...
              {activeDemos.map((demo) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'zelda-map-card relative text-left p-5 sm:p-6 border transition-all duration-300',
                      isCurrent && 'zelda-map-card-active',
                    )}
                    style={{
                      background: isCurrent
                        ? 'rgba(212, 184, 150, 0.92)'
                        : done
                          ? 'rgba(212, 184, 150, 0.85)'
                          : 'rgba(212, 184, 150, 0.65)',
                      borderColor: done
                        ? 'var(--zelda-gold)'
                        : isCurrent
                          ? 'var(--zelda-gold)'
                  ...
                  >
                    {/* Gold rune accent bar at top for active card */}
                    {isCurrent && (
                      <div
                        className="absolute top-0 left-[12px] right-[12px] h-[2px]"
                        style={{ background: 'linear-gradient(90deg, transparent, var(--zelda-gold), transparent)' }}
                      />
                    )}

                    {/* Corner rune accents - parchment/dungeon map style */}
                    <div className="absolute top-0 left-0 w-4 h-4" style={{ borderTop: '2px solid var(--zelda-gold)', borderLeft: '2px solid var(--zelda-gold)', opacity: (done || isCurrent) ? 0.8 : 0.3 }} />
                    <div className="absolute top-0 right-0 w-4 h-4" style={{ borderTop: '2px solid var(--zelda-gold)', borderRight: '2px solid var(--zelda-gold)', opacity: (done || isCurrent) ? 0.8 : 0.3 }} />
                    <div className="absolute bottom-0 left-0 w-4 h-4" style={{ borderBottom: '2px solid var(--zelda-gold)', borderLeft: '2px solid var(--zelda-gold)', opacity: (done || isCurrent) ? 0.8 : 0.3 }} />
                    <div className="absolute bottom-0 right-0 w-4 h-4" style={{ borderBottom: '2px solid var(--zelda-gold)', borderRight: '2px solid var(--zelda-gold)', opacity: (done || isCurrent) ? 0.8 : 0.3 }} />

                    {/* Status indicator */}
                    <div className="flex items-center gap-2 mb-3">
                      {done ? (
                        <span style={{ color: 'var(--zelda-gold)' }}>
                          {/* Triforce piece for completed */}
                          <svg width="18" height="16" viewBox="0 0 18 16">
                            <polygon points="9,0 18,16 0,16" fill="var(--zelda-gold)" stroke="#B8860B" strokeWidth="0.5" />
                          </svg>
                        </span>
                      ) : (
                        <span
                      ...
                      {done && stars > 0 && (
                        <span
                          ...
                          style={{ color: 'var(--zelda-gold)', fontFamily: 'var(--font-oxanium), sans-serif' }}
                        >
